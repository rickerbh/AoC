...
execute p xs =
  case parse p "test" xs of
    ...
    Prelude.Right addresses -> show $ length $ filter addressSupportsTLS $ addressesCombiner addresses
...
possibleAbbas :: [a] -> [([a], [a])]
possibleAbbas xs = let
  l = windows 2 xs
  in zip l (drop 2 $ l)

windows :: Int -> [a] -> [[a]]
windows n xs = filter (\x -> length x >= n) $ map (take n) (tails xs)

-- Gross intermediate data handling code

-- There has to be a nicer way than this. I think I probably need to learn about parser generators or something.
data Ip7AddressFragment = Ip7AddressFragment { primary :: String, hypernet :: String } deriving Show

addressesCombiner :: [[Ip7AddressFragment]] -> [Ip7Address]
addressesCombiner xs = map (\x -> fragmentsCombiner x) xs

fragmentsCombiner :: [Ip7AddressFragment] -> Ip7Address
fragmentsCombiner xs = foldl (\acc x -> Ip7Address ((primaries acc) ++ [(primary x)]) (hypernets acc ++ [(hypernet x)])) (Ip7Address [] []) xs

-- Parsers

addressesParser :: Parser [[Ip7AddressFragment]]
addressesParser = do
  a <- many1 ip7Parser
  return a

ip7Parser :: Parser [Ip7AddressFragment]
ip7Parser = do
  p <- primaryParser
  h <- option [] hypernetParser
  rest <- option [] ip7Parser
  optional endOfLine
  return (Ip7AddressFragment p h : rest)

primaryParser :: Parser String
primaryParser = many1 letter

hypernetParser :: Parser String
hypernetParser = do
  char '['
  h <- many1 letter
  char ']'
...
fullInput :: String
fullInput = [str|dnwtsgywerfamfv[gwrhdujbiowtcirq]bjbhmuxdcasenlctwgh
