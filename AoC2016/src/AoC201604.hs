...
sectorSums :: [Room] -> Int
sectorSums xs = sum $ map sectorId xs

validRooms :: [Room] -> [Room]
validRooms xs = filter isRoomValid xs

isRoomValid :: Room -> Bool
isRoomValid r = expectedChecksum r == checksum r

expectedChecksum :: Room -> Checksum
expectedChecksum r = map (\x -> fst x) $ take 5 $ frequencySort $ letterFrequency $ letters r
...
parsedRooms :: String -> Either ParseError [Room]
parsedRooms xs = parse roomsParser "test" xs
...
roomParser = do
  optional spaces
  l <- many1 lettersParser
  s <- many1 digit
  char '['
  c <- many1 letter
  char ']'
  optional endOfLine
  return $ Room (concat l) (read s :: Int) c

lettersParser :: Parser String
lettersParser = do
  s <- many1 letter
  optional $ char '-'
  return s
...
fullInput :: String
fullInput = [str|gbc-frperg-pubpbyngr-znantrzrag-377[rgbnp]
